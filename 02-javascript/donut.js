...
function renderFrame() {
    ...
    const zBuf = Array(SCREEN_WIDTH * SCREEN_HEIGHT).fill(0.0);
    ...
    for (let theta = 0.0; theta < 2.0 * Math.PI; theta += 0.07) {
        const cosTheta = Math.cos(theta);
        const sinTheta = Math.sin(theta);

        // phi loops over the center revolution of the torus
        for (let phi = 0.0; phi < 2.0 * Math.PI; phi += 0.02) {
            ...
            if (xp >= 0 && xp < SCREEN_WIDTH && yp >= 0 && yp < SCREEN_HEIGHT) {
                if (L > 0.0) {
                    ...
                    if (ooz > zBuf[idx]) {
                        zBuf[idx] = ooz;
                        const shadeIdx = Math.floor(L * 8.0);
    ...
    // Draw the frame: move cursor to top-left (\x1b[H) and print the screen buffer
    let output = "\x1b[H";
